import React,{useState} from 'react';
import { CiHome } from "react-icons/ci";
import { IoIosInformationCircleOutline, IoIosHelpBuoy } from "react-icons/io";
import { RiAccountPinCircleFill, RiLogoutCircleRLine } from "react-icons/ri";
import { IoSettingsOutline } from "react-icons/io5";
import { FcMenu } from "react-icons/fc";
import { LuContact2 } from "react-icons/lu";

export default function Navbar({setVal}) {

    const links = 
      "data:image/png;base64,";

    const lists = [
        {
          name: "Home",
          icon: <CiHome />,
          click: "home"
        },
        {
          name: "About",
          icon: <IoIosInformationCircleOutline />,
          click: "about"
        },
        {
          name: "Contact",
          icon: <LuContact2 />,
          click: "contact"
        },
        {
          name: "Help",
          icon: <IoIosHelpBuoy />,
          click: "help"
        },
      ];

    const newList = [
        {
            name: "Profile",
            icon: <RiAccountPinCircleFill />,
            click: "profile"
          },
          {
            name: "Edit Profile",
            icon: <IoSettingsOutline />,
            click: "sign"
          },
          {
            name: "Settings",
            icon: <IoSettingsOutline />,
            click: "sign"
          },
          {
            name: "Log Out",
            icon: <RiLogoutCircleRLine />,
            click: "home"
          }
    ]

    const [open,setOpen] = useState(false)


  return (
    <header className="bg-Hero bg-cover font-[Poppins] md:bg-top bg-center">
        <nav className='bg-white shadow-xl'>
            <div className='flex items-center font-medium justify-around'>
                <div className='z-50  md:w-auto w-full flex justify-between items-center'>
                    <img src="https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcTfmjD-QZD8Jtf40mHT6X1DOkQQdWG2YszoV45fiPHynIOgEPwH" alt="" className='md:cursor-pointer h-[40px] shrink-0'/>
                    {open===false && <div className ='text-3xl p-5 mr-5 shrink-0 md:hidden' onClick={()=>setOpen(!open)}>
                        <FcMenu/>
                    </div>}
                    {open===true && <div className='z-50 mr-9 md:hidden shrink-0 flex items-center'>
                        <img src="data:image/png;base64," alt="" srcSet="" className='h-[50px] md:mr-6 w-[50px] h-[50px]'/>
                    </div>}
                </div>
                <ul className='md:flex hidden uppercase items-center gap-8 font-[Poppins] '>
                    {lists.map((list, index) => {
                        return (
                        <div key={index} className="">
                            <div className="md:cursor-pointer px-3 text-left flex hover:text-[red] cursor-pointer items-center gap-1 ">
                                <h1>{list.icon}</h1>
                                <h1>{list.name}</h1>
                            </div>
                        </div>
                     );
                   })}
                </ul>
                {/* Mobile Navigation */}
                <div className={`md:hidden bg-white absolute w-full bottom-0 py-24 pl-4 duration-500 ${open ? 'left-0' : 'left-[-100%]'}`}>
                    {lists.map((list, index) => {
                        return (
                        <div key={index} className="">
                            <div className="md:cursor-pointer px-3 text-left flex hover:text-[red] cursor-pointer items-center gap-1 py-7">
                                <h1>{list.icon}</h1>
                                <h1>{list.name}</h1>
                            </div>
                        </div>
                     );
                   })}
                   <div className='py-5'>
                        <button className='hover:bg-[#027bfd] bg-[#68b0fd] border-none text-[#fff] cursor-pointer mx-3 px-6 py-2 rounded-full hover:text-[red]' onClick={()=>setOpen(!open)}>Cancel</button>
                    </div>
                </div>
                
                <div className='md:inline hidden z-50 shrink-0 flex items-center group '>
                    <img src={links} alt="" srcSet="" className='my-[10px] md:mr-6 w-[50px] h-[50px] text-center'/>
                    <div className='absolute top-15 shadow-xl hidden group-hover:block hover:block rounded-[10px] opacity:0'>
                        <div className='py-3 '>
                            <div className='w-4 h-4 left-3 absolute mt-1 bg-white rotate-45 border-l border-t rounded-[2px] bg-[#F5F5F5]'></div>
                        </div>
                        <div className='bg-[#F5F5F5] p-3.5 rounded-[10px] border'>
                            {newList.map((item,index)=>{
                                return (
                                    <div key={index} onClick={()=>{setVal(item.click)}} className='flex items-center text-gray-600 text-sm hover:text-[red] cursor-pointer bg-[#F5F5F5] hover:font-semibold '>
                                        <h1 className='m-1.5 text-[20px]'>{item.icon}</h1>
                                        <h1>{item.name}</h1>
                                    </div>
                                )
                            })
                            }
                        </div>
                    </div>
                </div>

            </div>
        </nav>
    </header>
  )
}
